...
def main():
    ...
    if len(sys.argv) == 3:
        ...
        print('loading data...')
        Y, genre_df, pred_actual_df = load_data(database_filepath, model)

        print('creating visuals...')
        create_visuals(Y, genre_df, pred_actual_df)
...
def load_data(database_filepath, model):
    """
    Load and format data necessary for visuals
    :param database_filepath:
    :param model:
    :return: Y, genre_df, pred_actual_df
    """
    ...
    # create df for predicted vs actual counts visual
...
def create_visuals(Y, genre_df, pred_actual_df):
    ...
    Y_counts = [Y[c].sum() for c in Y.columns]
    ...
    plt.bar(x=genre_df['genre'].value_counts().index, height=genre_df['genre'].value_counts(), color='b')
    plt.xlabel('Genre')
    plt.ylabel('Counts')
